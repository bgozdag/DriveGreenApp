...
censored_values = [
    "WifiPassword", "password", "apnName", "apnUsername", "apnPassword", "simPin"
]
...
class BluetoothHandler:
    ...
    def get_local_bdaddr(cls):
        try:
            local_bdaddr = str(read_local_bdaddr()).strip("[]'").replace(":", "")
            return local_bdaddr
    ...
    def setDisconnected(self):
        ...
        msg = json.dumps(msg)
        # TODO send disconnection status
        self.connectedBluetoothServerIndex = None
        self.__activateAll()
    ...
    def stop(self):
        ...
        system("echo disconnect | bluetoothctl")
        self.running = False
        self.connectedBluetoothServerIndex = None
        for server in self.bluetoothServers:
            server.deactivate()
    ...
    def onReceive(self, message):
        if not any(value in message for value in censored_values):
